...
class MultiHeadAttention(nn.Module):
    ...
    def __init__(self, d_model: int, num_attention_heads: int):
        ...
        self.heads = nn.ModuleList([
            AttentionHead(d_model, d_model // num_attention_heads, d_model // num_attention_heads, d_model // num_attention_heads)
    ...
    def forward(self, hidden_state, mask=None):
        ...
        x = [head(hidden_state, mask) for head in self.heads]
        x = torch.cat(x, dim=-1)
...
class TransformerDecoderLayer(nn.Module):
    ...
    def forward(self, x: torch.Tensor, mask: torch.Tensor) -> torch.Tensor:
        ...
        attn_output = self.self_attention(norm_x, mask)
        x = x + attn_output
